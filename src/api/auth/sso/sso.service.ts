...
export class SsoService {
	...
	public async getAvailableMethods() {
		return ['google', 'discord', 'telegram']
	}
	...
	public getTelegramAuthUrl(action: 'login' | 'connect') {
		...
		url.searchParams.append('origin', 'http://localhost:4000')
		url.searchParams.append('embed', '1')
		url.searchParams.append('request_access', 'write')
		url.searchParams.append(
			'return_to',
			action === 'connect'
				? 'http://localhost:4000/account/connections'
				: 'http://localhost:4000/auth/telegram-oauth-finish'
		)
	...
	public async connectWithTelegram(dto: TelegramAuthRequest, user: User) {
		...
		if (existing)
			throw new ConflictException(
				'Этот Telegram аккаунт уже привязан'
			)
